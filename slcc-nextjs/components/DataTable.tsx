...
export default function DataTable({
  ...
  totalPages = 1, onPageChange, onRefresh, emptyMessage = 'No records found', onEdit
}: DataTableProps) {
        ...
        <div className="overflow-x-auto">
          <table className="w-full data-table">
            ...
            <tbody className="divide-y divide-slate-800/50 bg-slate-900/40">
              {data.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + (onEdit ? 1 : 0)} className="text-center text-slate-600 py-12">
                    {emptyMessage}
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={i} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    {onEdit && (
                      <td className="text-right">
                        <button
                          onClick={() => onEdit(row)}
                          className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 rounded-lg transition-colors"
                        >
                          <Pencil size={12} /> Edit
                        </button>
                      </td>
